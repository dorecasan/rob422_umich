...
def get_translation_jacobian(robot, joint_indices):
    ...
    for i, idx  in enumerate(joint_indices):
        joint_pos = get_joint_position(robot, idx)
        joint_axis = get_joint_axis(robot, idx)
        J[:,i] = np.cross(joint_axis,ee_pos - joint_pos)
...
def get_jacobian_pinv(J):
    ...
    J_pinv = J.T @ np.linalg.inv(J @ J.T + gamma*I) 
...
def main():
    ...
    alpha = 0.5
    while True:
        ee_pos = get_ee_transform(robot, joint_idx)[:3,3]
        pos_error = target - ee_pos
        if np.linalg.norm(pos_error) < goal_tolerance:
            break
        Jacco = get_translation_jacobian(robot, joint_idx)
        invJacco = get_jacobian_pinv(Jacco)
        joint_error = invJacco @ pos_error
        
        if (np.linalg.norm(joint_error) > alpha):
            joint_error = alpha * joint_error / np.linalg.norm(joint_error)
        
        q = q + joint_error
        
        for i, joint_name in enumerate(joint_names):
            upp = joint_limits[joint_name][1]
            low = joint_limits[joint_name][0]
            q[:,i] = min(max(q[:,i],low),upp)
