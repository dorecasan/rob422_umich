...
def linear_constraint(x,a,b):
  return float(-a.T*x + b)

def d_linear_constraint(x,a,b):
  return -a

def log_linear_constraint(x,a,b):
  return np.log(linear_constraint(x,a,b))

def d_log_constraint(x,a,b):
  return -1/linear_constraint(x,a,b)*d_linear_constraint(x,a,b)

def dd_log_constraint(x,a,b):
  f = linear_constraint(x,a,b)
  df = d_linear_constraint(x,a,b)
  return 1/(f**2)*df @ df.T
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        while 1:
            ...
            fprimeprime = t*np.zeros([c.size, c.size])

            #compute the first and second derivatives from each hyperplane and aggregate
            for j in range(0,numplanes):
                fprime_for_plane_j = d_log_constraint(x,a[:,j],b[j])

                fprimeprime_for_plane_j = dd_log_constraint(x,a[:,j],b[j])
