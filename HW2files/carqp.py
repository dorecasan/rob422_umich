...
def linearize_dynamics_numerically(x_r, u_r, h, true_dynamics):
    ...
    for i in range(n):
      delta_h = np.zeros(n) 
      delta_h[i] = delta_h[i] + h 
      Jacobian[:,i] = (f(xu_r+delta_h) - f_r)/h 
...
def optimize_single_action(goal_state, current_state, reference_control, A, B, speed_limit, turn_limit):
    ...
    ### YOUR CODE HERE ###
    constraints = []
    constraints.append(control[0] <= speed_limit[1])
    constraints.append(control[0] >= speed_limit[0])
    constraints.append(control[1] <= turn_limit[1])
    constraints.append(control[1] >= turn_limit[0])
    objective_fnc = cvx.Minimize(cvx.sum_squares(current_state + B @(control - reference_control) - goal_state))

    ### YOUR CODE HERE ###
    prob = cvx.Problem(objective_fnc, constraints)
    result = prob.solve()
